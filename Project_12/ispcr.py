...
def ispcr(primer_file: str, assembly_file: str, max_amplicon_size: int) -> str:
	"""
	Unifying the steps from the isPCR program to run all three in non-stepwise manner. 
	"""
...
def ispcr_seq(fasta_string = str) -> dict[str:str]:
	"""
	Convert fasta string into seq, combine with next functions to make the dictionary
	"""
	sequence = {}
	header = 'x'
	splits = []
...
def create_dict(primer_file: str, assembly_file: str, max_amplicon_size: int):
	return ispcr_seq(ispcr(primer_file, assembly_file, max_amplicon_size))
