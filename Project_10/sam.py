...
# >0 if it is true or a 0 if it is false
        ...
        self.is_primary = (self.flag & 256) == 0 and (self.flag & 2048) == 0
...
#Write a base_at_pos method to take a 1-base position in the reference sequence and return the base mapped to specific position in the reference.
    def base_at_pos(self, pos: int) -> str:
        #for unmapped cigar or position out of range
        if self.cigar == "*" or pos < self.pos:
            return ""
        
        reference = self.pos
        last_ref = reference #keep track of last match
        reading = 0
        num = ''
        ...
        for unit in self.cigar:
            if unit.isdigit():
                ...
                if letter == "M":
                    ...
                    last_ref = reference + number - 1
                    reading = reading + number
                    reference = reference + number
                
                #add  on until matches are found
                elif letter == "I":
                    if last_ref == pos or reference == pos:
